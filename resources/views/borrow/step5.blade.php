<x-step-layout title="Verifikasi Siswa" :currentStep="5" action="Peminjaman">
    ...
    <div class="w-full max-w-175 bg-white border border-accent rounded-[20px]
        ...
        <div class="flex items-start gap-3 bg-accent/60 rounded-xl px-5 py-4 mb-8">
            ...
            <p class="font-inter text-sm text-gray-700 leading-relaxed">
                ...
                dari kelas <span class="font-semibold text-primary">{{ $grade }} - {{ $class->class_name }}</span>
            </p>
        </div>
        ...
        {{-- Form --}}
        <form action="{{ route('borrow.step5.verify') }}" method="POST">
            @csrf
